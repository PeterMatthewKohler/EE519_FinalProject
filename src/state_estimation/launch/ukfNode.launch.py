...
def generate_launch_description():
    ...
    # ----------- Generate Launch Actions for Unscented Kalman Filter node -----------
    share_dir = get_package_share_directory(PACKAGE)

    # Each node call is a new object in the Launch Description array.
    ukf_node = Node(                      # Node 1 object init
        package=PACKAGE,                      # Package name
        executable="ukfNode",               # Exe name, as declared in CMakeLists.txt
        name="ukf_node",                  # Node name, must be unique
